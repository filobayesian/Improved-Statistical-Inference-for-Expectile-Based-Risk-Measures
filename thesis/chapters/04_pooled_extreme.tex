...
\todo{Rewrite this as a bridge from
\Cref{sec:bg:weissman-expectile} and \Cref{ch:pool-int}. Explain that
very-extreme levels satisfy
$n(1-\tau_n^\prime)\to c\in[0,\infty)$, so direct LAWS or direct QB
estimation at $\tau_n^\prime$ is not the object of the thesis. The
only admissible input remains the Paper-3 primitive pair
$(\hat\gamma_j(k_j),\hat q_j^\star(\,\cdot\,\mid k_j))$ from each
machine; no expectile-specific primitive is introduced in Chapter 4.
The chapter therefore extrapolates the canonical pooled QB estimator of
\Cref{eq:pool-int:expectile-pool}, exactly as
\Cref{prop:bg:plug-in-clt} says one may extrapolate any
intermediate-level expectile estimator with a joint root-rate limit.}
